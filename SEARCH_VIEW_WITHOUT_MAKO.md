...
> **Status, 2026-09-22.** The candidate this note chose, a classic dashboard carrying a
> search form, was withdrawn before merge (see "The decision") and the app now ships no
> 10x Search tab at all: Splunk's own search bar is served by the `tenxsearch` command,
> and the browser hook stays for classic dashboards. The measurements below are why, and
> they are what stops the tab being rebuilt. `tests/live_render.js`, the harness that
> took them, is in this repo.
