...
//
// A healthy dashboard in this app reads: loadingStuck false, dashboardBody true,
// hookInstalled true, searchJobs.rewritten true, and with a term, termOccurrences above
// zero. Wait for splunkd's management port to answer before rendering after a restart,
// or the page sees 502s from splunkweb's proxy and cannot create a job.
//
// Measured on 10.4.3 against a dashboard whose panels carry no expansion macro: four
// panels post to Splunk's own /search/v2/jobs first, the hook installs, execute(true)
// restarts them, and all four re-post to /tenx-search. Both counts are reported, because
// only the second says the hook did its work.
...
(async () => {
  ...
  // An existing session, SSO, or a Free licence (which has no authentication at all)
  // lands on the app with no form to fill. Only log in when there is a form.
  ...
  // A dashboard's panels mount well after load, so a flat wait reports a page that is
  // still rendering as a page that is blank. Wait for something to appear, then settle.
  // A genuinely blank page spends this timeout and is reported with loadingStuck true.
  await page.waitForSelector('.dashboard-body, .search-bar, textarea.search-field', { timeout: 60000 }).catch(() => {});
  await page.waitForTimeout(8000);

  // Splunk's first-run dialogs (telemetry opt-in, the page tour) sit over the page and
  // swallow clicks meant for the search input.
  await page.evaluate(() => document
    .querySelectorAll('[role="dialog"][aria-modal="true"], .modal-backdrop, .opt-in-dialog')
    .forEach(e => e.remove())).catch(() => {});
  ...
  // Counts, not a truncated list. The hook installs after the panels have already
  // dispatched and then restarts them, so the rewritten jobs are the LAST posts the page
  // makes: a list cut at the first few shows only Splunk's own endpoint and reads as a
  // hook that never fired.
  const toTenxSearch = posted.filter(u => /\/tenx-search(\?|$)/.test(u));
  const toSplunkJobs = posted.filter(u => /\/search\/(v2\/)?jobs(\?|$)/.test(u));

  console.log(JSON.stringify({
    view, url, signals, searchResult,
    searchJobs: {
      toTenxSearch: toTenxSearch.length,
      toSplunkJobs: toSplunkJobs.length,
      rewritten: toTenxSearch.length > 0,
      sample: toTenxSearch.concat(toSplunkJobs).map(u => u.replace(BASE, '')).slice(0, 4),
    },
    consoleErrors: consoleErrors.slice(0, 5),
    failedResponses: failed.slice(0, 8),
    allPosts: posted.map(u => u.replace(BASE, '').slice(0, 100)),
  }, null, 2));
  await browser.close();
